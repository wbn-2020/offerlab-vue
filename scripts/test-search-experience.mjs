...
assert.match(source, /结果会受到当前筛选条件和可检索范围影响/, 'SearchView must explain bounded search coverage without exposing infrastructure terms')
...
assert.match(source, /结果可能不完整|备用搜索方式/, 'SearchView must explain degraded search in user-facing language')
