...
assert.match(source, /search-source-notice/, 'SearchView must render user-facing search notices')
assert.doesNotMatch(source, /searchResultMeta/, 'SearchView must not retain per-request infrastructure metadata')
assert.doesNotMatch(source, /fallbackReasonText/, 'SearchView must not render raw backend fallback reasons')
...
assert.match(source, /const countText = resultDisplayCapReached\.value[\s\S]*hasMore\.value[\s\S]*已加载 \$\{resultCount\.value\} 条内容，继续加载可查看更多/, 'SearchView must tell users when more post results can be loaded without overstating capped results')
...
assert.match(source, /const searchStatusText\s*=\s*computed/, 'SearchView must derive the public search status text')
assert.match(source, /searchStatus\.value\?\.available === false/, 'SearchView must distinguish complete public search outage from a normal state')
...
assert.match(source, /暂时无法读取搜索状态，仍可浏览热门内容、作者和发现页。/, 'SearchView must explain local fallback actions when status cannot be loaded')
