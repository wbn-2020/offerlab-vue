...
assert.doesNotMatch(topicDetail, /发布相关技术经验|后台为专题绑定/, 'topic empty state must not push technical-only publishing')
assert.doesNotMatch(topicTypeMap, /return '技术栈'/, 'legacy tech_stack topic type must be remapped for general community display')
assert.match(topicDetail, /去发现内容[\s\S]*搜索相似内容[\s\S]*发布内容/, 'topic empty state must link to discover, search, and publish')
assert.match(topicDetail, /filter-scroll-hint[\s\S]*左右滑动/, 'topic filters must expose a visible mobile horizontal-scroll hint')

const tagHeader = firstRegion(tagDetail, '<section class="identity-banner"')
const tagFilter = firstRegion(tagDetail, '<section class="content-toolbar"')
assert.match(tagHeader, /标签索引/, 'tag detail header must use general tag index language')
assert.match(tagHeader, /经验分享、问题求助、攻略清单、资源推荐、复盘记录和观点讨论/, 'tag detail description must cover general content types')
assert.match(tagFilter, /标签内容筛选/, 'tag detail filter section must not be called topic filtering')
assert.doesNotMatch(tagHeader, /专题 \/ 技术栈|技术文章/, 'tag detail must not look like a technical tag page')
assert.match(tagDetail, /发现内容[\s\S]*搜索相关标签[\s\S]*发布内容/, 'tag empty state must link to discover, search, and publish')
assert.match(tagDetail, /filter-scroll-hint[\s\S]*左右滑动/, 'tag filters must expose a visible mobile horizontal-scroll hint')

const legacyExploreStart = explore.indexOf('<section v-if="false')
const visibleExplore = legacyExploreStart === -1 ? explore : explore.slice(0, legacyExploreStart)
...
assert.match(explore, /activeEntryPostTypeSet/, 'explore must support entries that aggregate multiple post types')
...
assert.match(explore, /Promise\.allSettled\(activeEntryPostTypes\.value\.map/, 'multi-type entries must fetch each post type instead of sampling one mixed page')
assert.match(visibleExplore, /explore-channel-strip/, 'explore must keep one compact channel and content-form entry')
assert.doesNotMatch(visibleExplore, /explore-channel-plaza|explore-side-nav|explore-mobile-topics/, 'explore must not repeat channel and topic navigation structures')
assert.doesNotMatch(visibleExplore, /\/channels\/|\/channel\//, 'phase 2 must not default to a new standalone channel page')
...
assert.match(domains, /COMMUNITY_CONTENT_FORMS/, 'domain utils must expose independent content forms')
assert.match(domains, /key:\s*'investment'/, 'investment must be a formal community channel')
assert.doesNotMatch(domains, /ALL_COMMUNITY_CHANNELS[\s\S]*resources/, 'resource entry must not remain in the formal channel list')
assert.doesNotMatch(domains, /ALL_COMMUNITY_CHANNELS[\s\S]*qa-discussion/, 'question entry must not remain in the formal channel list')
