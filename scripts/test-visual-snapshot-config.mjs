...
assert.match(runner, /waitForVisualReady/, 'visual snapshot runner must wait for an explicit stable application state')
assert.match(runner, /pendingApiRequests/, 'visual snapshot runner must wait for foreground API requests to settle')
assert.match(runner, /stableSamples >= 3/, 'visual snapshot runner must observe repeated stable layout samples before capture')
assert.match(runner, /captureError/, 'visual snapshot runner must retain route-level navigation and readiness failures in summary.json')
assert.match(runner, /expectsDarkSurface && check\.noLightSurface/, 'light-surface regression checks must only apply when the effective theme is dark')
assert.match(runner, /visualTheme === 'light' && item\.darkMode !== false/, 'visual snapshot runner must detect an incorrect dark class in explicit light mode')
...
assert.match(packageJson, /"playwright": "\^1\.61\.1"/, 'package devDependencies must declare the current Playwright baseline for screenshot evidence')
