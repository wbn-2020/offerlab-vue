...
const governanceDisplay = readSource('src/utils/governanceDisplay.ts')
...
assert.match(postDetail, /REPORT_REASON_OPTIONS/, 'Report dialog must render the shared report reason options.')
assert.match(governanceDisplay, /value:\s*'ILLEGAL'/, 'Shared report reasons must include illegal-content reporting.')
assert.match(governanceDisplay, /value:\s*'MISLEADING'/, 'Shared report reasons must include misleading-content reporting.')
assert.match(governanceDisplay, /value:\s*'RISK'/, 'Shared report reasons must include high-risk content reporting.')
