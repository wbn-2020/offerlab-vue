...
assert.match(textQuality, /\\u00E6/, 'text-quality module must reject latin-1 mojibake marker æ in restored draft text')
assert.match(textQuality, /\\u00C3/, 'text-quality module must reject latin-1 mojibake marker Ã in restored draft text')
assert.match(textQuality, /\\u9427/, 'text-quality module must reject Chinese mojibake marker 鐧 in restored draft text')
assert.match(textQuality, /\\u6769/, 'text-quality module must reject Chinese mojibake marker 杩 in restored draft text')
assert.match(textQuality, /\\u7459/, 'text-quality module must reject Chinese mojibake marker 瑙 in restored draft text')
assert.match(textQuality, /\\u93BB/, 'text-quality module must reject Chinese mojibake marker 鎻 in restored draft text')
...
assert.match(editor, /normalizeDomain\(\s*draft\.domain\s*\?\?\s*extension\.domain\s*\?\?\s*\(extension\.anonymous\s*\?\s*DOMAIN\.CAREER\s*:\s*DOMAIN\.TECH\)\s*\)/, 'server draft restore must prefer explicit domain and fallback to extJson.domain')
...
assert.match(editor, /to="\/admin\/tags"/, 'editor must route admins/operators to tag governance when publish fails')
...
assert.match(editor, /知识沉淀辅助/, 'editor must expose local knowledge-assist guidance for community posts')
