...
assert.match(editor, /readLocalDraft/, 'editor must retain local fallback draft detection')
assert.match(editor, /queueLocalDraftRestore/, 'editor must queue a local fallback draft for an explicit user decision')
assert.match(editor, /restorePendingLocalDraft/, 'editor must retain an explicit local fallback draft restore action')
