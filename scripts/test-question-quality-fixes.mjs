...
assert.match(detail, /<LoadingSkeleton v-if="isQuestionLoading"/, 'QuestionDetailView must not keep showing skeleton after an error response')
assert.match(detail, /isQuestionLoading\s*=\s*computed\(\(\)\s*=>\s*isLoading\.value\s*&&\s*!isError\.value\)/, 'QuestionDetailView loading state must yield to error/not-found states')
assert.match(detail, /v-else-if="isError"/, 'QuestionDetailView must distinguish load errors from not-found empty state')
assert.match(detail, /retry:\s*false/, 'QuestionDetailView must not leave not-found responses in retrying skeleton state')
assert.match(detail, /isQuestionNotFound/, 'QuestionDetailView must render a dedicated recoverable not-found state')
assert.match(detail, /题目不存在或已被删除/, 'QuestionDetailView must explain a missing question clearly')
assert.match(detail, /搜索相似内容/, 'QuestionDetailView not-found state must offer a search recovery action')
...
assert.match(detail, /当前知识库还没有同步到详情页/, 'QuestionDetailView empty state must explain backend/detail sync gaps')
...
assert.match(questions, /当前还没有可见题目/, 'QuestionsView must explain the true empty question-library state')
assert.match(questions, /发布内容/, 'QuestionsView empty state must offer a content creation recovery action')
assert.match(questions, /hasActiveFilters/, 'QuestionsView must distinguish empty filtered results from an empty library')
assert.match(questions, /v-else-if="listError"/, 'QuestionsView must render API errors separately from empty results')
assert.match(questions, /getErrorMessage\(error,\s*'知识库暂时无法加载，请稍后重试。'\)/, 'QuestionsView must use normalized API error messages')
