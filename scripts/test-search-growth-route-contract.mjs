...
assert.match(searchView, /filters\s*=\s*reactive<\{ q: string; domain\?: number;/, 'SearchView must keep domain filter state for post search')
...
assert.match(searchView, /return Boolean\(filters\.q \|\| filters\.domain \|\| filters\.company \|\| filters\.position \|\| filters\.type\)/, 'SearchView query state must treat domain as a real post-search filter')
