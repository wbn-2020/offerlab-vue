...
assert.match(detail, /加入知识复盘/, 'question detail must expose the focused knowledge-review CTA')
assert.match(detail, /const prepReturnLink\s*=\s*computed/, 'question detail must build a prep desk return link')
assert.match(detail, /回学习空间/, 'question detail must expose learning-space return CTA')
...
assert.match(workspace, /to="\/me\/prep"/, 'completed practice must link back to learning space')
...
assert.match(workspace, /继续知识复盘/, 'workspace must expose a next focused knowledge-review CTA')
