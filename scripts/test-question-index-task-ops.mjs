...
assert.match(view, /questionIndexPollGeneration/, 'OpsView question-index polling must invalidate stale in-flight responses')
assert.doesNotMatch(view, /window\.setInterval/, 'OpsView async task polling must not use overlapping intervals')
assert.match(view, /window\.setTimeout\(async \(\) =>/, 'OpsView question-index polling must schedule only after the previous request finishes')
