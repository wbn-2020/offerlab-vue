...
assert.match(viewSource, /actionText="去题库看看"/, 'MePrep unavailable empty state must offer a concrete next action')
assert.match(viewSource, /isPrepStarterEmpty/, 'MePrep must detect the all-zero starter state')
assert.match(viewSource, /先把准备台喂进第一批数据/, 'MePrep starter state must explain how to create value')
assert.match(viewSource, /href="#prep-targets"/, 'MePrep starter state must jump to the target form')
assert.match(viewSource, /添加备考目标/, 'MePrep target form submit should explain the action instead of saying only add')
assert.match(viewSource, /先添加目标公司、岗位或技术标签/, 'MePrep empty target copy must mention all supported target types')
