...
assert.match(homeView, /参与问答/, 'HomeView must route new users into community Q&A')
assert.match(homeView, /发布内容/, 'HomeView must route new users into general publishing')
assert.match(homeView, /实验工具/, 'HomeView must keep legacy tools reachable without making them the primary positioning')
assert.match(homeView, /d\.domain\s*!=\s*null\s*&&\s*d\.domain\s*===\s*activeDomain/, 'HomeView must not highlight post-type-only channels on the aggregate feed')
