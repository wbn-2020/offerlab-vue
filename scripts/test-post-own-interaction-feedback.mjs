...
assert.match(postDetail, /target\.counter\.like = Math\.max\(0, target\.counter\.like \+ \(liked \? -1 : 1\)\)/, 'like counter must be updated optimistically on the request-owned post')
assert.match(postDetail, /target\.counter\.like = Math\.max\(0, target\.counter\.like \+ \(liked \? 1 : -1\)\)/, 'like counter must roll back when the request fails')
assert.match(postDetail, /target\.counter\.favorite = Math\.max\(0, target\.counter\.favorite \+ \(favorited \? -1 : 1\)\)/, 'favorite counter must be updated optimistically on the request-owned post')
assert.match(postDetail, /target\.counter\.favorite = Math\.max\(0, target\.counter\.favorite \+ \(favorited \? 1 : -1\)\)/, 'favorite counter must roll back when the request fails')
