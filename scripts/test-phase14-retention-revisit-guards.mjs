...
assert.match(retentionApi, /notificationApi\.getList\(undefined/, 'discussion revisits must load a bounded in-site notification set.')
assert.match(retentionApi, /notificationApi\.getPreferences\(\)/, 'fallback retention summary must load persisted notification preferences.')
assert.match(retentionApi, /normalizeNotificationPreference/, 'fallback retention summary must normalize persisted notification preferences.')
assert.match(retentionApi, /!preferences\.interactionNotification/, 'discussion revisits must respect the global interaction preference.')
for (const preference of ['commentNotification', 'mentionNotification', 'favoriteNotification', 'systemNotification']) {
  assert.match(retentionApi, new RegExp(`preferences\\.${preference}`), `fallback retention summary must respect ${preference}.`)
}
assert.match(retentionApi, /notification_preferences_unavailable/, 'preference failures must explicitly degrade the fallback summary.')
assert.doesNotMatch(retentionApi, /后端清单未稳定时使用收藏兜底/, 'fallback copy must not disguise all favorites as unorganized favorites.')
...
assert.match(revisitPanel, /不是独立后端留存服务/, 'revisit panel must disclose that the summary is a frontend aggregation.')
