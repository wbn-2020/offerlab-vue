...
assertRiskBeforeCall(opsView, 'retrySelectedMessages', /opsApi\.retryOutboxBatch\(ids, note, preview\.previewNonce\)/, 'batch outbox retry')
assertPreviewBeforeRiskBeforeCall(opsView, 'retrySelectedMessages', /opsApi\.previewOutboxRetryBatch\(ids\)/, /opsApi\.retryOutboxBatch\(ids, note, preview\.previewNonce\)/, 'batch outbox retry preview')
...
assertRiskBeforeCall(opsView, 'replayFailedSearchIndexRetryTasks', /opsApi\.replaySearchIndexRetryTasks\(ids, note, preview\.previewNonce\)/, 'batch search index replay')
assertPreviewBeforeRiskBeforeCall(opsView, 'replayFailedSearchIndexRetryTasks', /opsApi\.previewSearchIndexRetryTasks\(ids\)/, /opsApi\.replaySearchIndexRetryTasks\(ids, note, preview\.previewNonce\)/, 'batch search index replay preview')
...
assertRiskBeforeCall(opsView, 'replayFailedNotificationRetryTasks', /opsApi\.replayNotificationRetryTasks\(ids, note, preview\.previewNonce\)/, 'batch notification replay')
assertPreviewBeforeRiskBeforeCall(opsView, 'replayFailedNotificationRetryTasks', /opsApi\.previewNotificationRetryTasks\(ids\)/, /opsApi\.replayNotificationRetryTasks\(ids, note, preview\.previewNonce\)/, 'batch notification replay preview')
assert.match(opsView, /const batchPreviewImpact = \(preview: BatchActionPreview\) => preview\.estimatedImpact \?\? preview\.eligible/, 'batch preview must prefer backend estimated impact')
assert.match(opsView, /预计影响：\$\{batchPreviewImpact\(preview\)\}/, 'batch risk context must show backend estimated impact')
assert.match(opsView, /后端风险原因：\$\{preview\.riskReason\}/, 'batch risk context must show backend risk reason')
assert.match(opsView, /impactCount: batchPreviewImpact\(preview\)/, 'batch risk impact count must use backend estimated impact')
assert.doesNotMatch(opsView, /impactCount: preview\.eligible/, 'batch risk impact count must not fall back to eligible directly')
assert.match(opsView, /outbox-mobile-list[\s\S]*class="icon-button danger-action desktop-risk-action"/, 'Outbox mobile cards must hide single retry as a desktop-only risk action')
...
assert.match(opsView, /本地开放仅限本机开发验证/, 'OpsView must explicitly warn that LOCAL_OPEN is development-only')
