...
assertBlockIncludes(domainLoaderBlock, /loadBackendReviewQueue\(false\)/, 'Scoped moderators must load the backend queue that is filtered by server-side source-domain authorization.')
...
assertBlockExcludes(domainLoaderBlock, /opsApi\.listModerationKeywords|opsApi\.listModerationHits|opsApi\.listModerationUsers|loadTopics|loadTags|postApi\.listAdminTopics|postApi\.listAdminTags/, 'Domain-only moderators must not trigger global governance loaders.')

const backendQueueBlock = constBlock('loadBackendReviewQueue')
assertBlockIncludes(backendQueueBlock, /if\s*\(!canModerate\.value\)[\s\S]*reviewQueueSource\.value\s*=\s*['"]frontend-fallback['"][\s\S]*return/, 'Backend review queue must require global or scoped moderation permission.')
assertBlockIncludes(backendQueueBlock, /opsApi\.listReviewQueue/, 'Authorized moderators must use the backend review queue.')
assertSource(governance, /filteredReviewQueueItems\s*=\s*computed\(\(\)\s*=>\s*reviewQueueItems\.value\.filter\([\s\S]*queueFilters\.domain[\s\S]*item\.domain/s, 'The selected queue domain must still be enforced in the visible frontend result set.')
assertSource(governance, /const queueDomain = \(item: BackendReviewQueueItem\)[\s\S]*queueExtJson\(item\)\.domain[\s\S]*isKnownDomain/s, 'Backend queue domains must be derived from a validated field or queue extJson.')
...
assertSource(governance, /canQueueAction\s*=\s*\(item:\s*ReviewQueueItem,\s*action:\s*ReviewQueueAction\)\s*=>\s*\{[\s\S]*if\s*\(!canModerate\.value\)\s*return false/s, 'Backend review queue action visibility must require global or scoped moderation permission.')
assertSource(governance, /item\.creatorUid && item\.creatorUid === currentUid[\s\S]*item\.assigneeUid && item\.assigneeUid !== currentUid/s, 'Queue actions must hide self-review and items claimed by another moderator.')
assertSource(governance, /previewedPendingPostIds\.value\.has\(pendingPostPreviewKey\(item\)\)/, 'Pending post decisions must require a successful preview of the current queued version.')
assertSource(governance, /handleReviewQueueAction\s*=\s*async\s*\(item:\s*ReviewQueueItem,\s*action:\s*ReviewQueueAction\)\s*=>\s*\{[\s\S]*if\s*\(!canModerate\.value\)\s*return/s, 'Backend review queue actions must require global or scoped moderation permission.')
