  ...
  <div class="maintenance-page min-h-screen">
    ...
    <main class="mx-auto max-w-5xl px-4 py-8">
      ...
      <div class="filter-bar">
        <select v-model="status" class="field-control" @change="load()">
...
const drafts = reactive<Record<string, { deliveryType: 'POST' | 'QUESTION' | 'SERIES'; deliveryRefId: string; note: string }>>({})

const draft = (task: ContentMaintenanceTask) => (
  drafts[String(task.id)] ||= { deliveryType: 'POST', deliveryRefId: '', note: '' }
)
...
.action-row { margin-top:.9rem;justify-content:flex-start; }.submit-form { display:grid;grid-template-columns:10rem minmax(0,1fr) auto;gap:.65rem;margin-top:1rem;border-top:1px solid rgb(241 245 249);padding-top:1rem; }.note-input { grid-column:1 / -1;resize:vertical; }.primary-button { display:inline-flex;min-height:38px;align-items:center;justify-content:center;border:1px solid rgb(8 145 178);border-radius:.5rem;background:rgb(8 145 178);padding:.5rem .75rem;color:white;font-size:.78rem;font-weight:900; }.primary-button:disabled,.icon-button:disabled { cursor:not-allowed;opacity:.5; }
...
@media (max-width:720px) { .page-header,.task-head { flex-direction:column; }.submit-form { grid-template-columns:1fr; }.note-input { grid-column:auto; } }
:global(.dark) .maintenance-page { background:rgb(2 6 23); }:global(.dark) .task-row,:global(.dark) .field-control,:global(.dark) .icon-button,:global(.dark) .secondary-button,:global(.dark) .state { border-color:rgb(51 65 85);background:rgb(15 23 42);color:rgb(203 213 225); }:global(.dark) .page-header h1,:global(.dark) .task-head h2 { color:rgb(248 250 252); }:global(.dark) .page-header span,:global(.dark) .detail,:global(.dark) .meta { color:rgb(148 163 184); }:global(.dark) .submit-form { border-color:rgb(51 65 85); }
</style>
