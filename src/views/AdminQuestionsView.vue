    ...
    <main class="mx-auto max-w-7xl px-4 py-8">
      ...
      <section class="mb-6 panel compact-filter-panel">
        <div class="filter-grid">
          ...
          <input v-model.trim="filters.sourcePostId" class="field-input" inputmode="numeric" placeholder="来源帖子 ID" @keyup.enter="applyFilters" />
          ...
          <div v-else class="space-y-3">
            ...
            >
              ...
              <div class="min-w-0 flex-1 text-left">
                ...
                <p v-if="question.qualityReason" class="mt-2 line-clamp-2 text-xs font-semibold text-amber-700 dark:text-amber-300">
                  {{ question.qualityReason }}
                </p>
              </div>
            </div>
          </div>
            ...
            <section class="duplicate-panel">
              <div class="mb-3 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p class="text-xs text-slate-500 dark:text-slate-400">同 normalized hash 题组，可调整主问题或隐藏重复来源。</p>
                </div>
              ...
              <div v-else class="space-y-2">
                ...
                <article v-for="item in duplicateGroup.questions" :key="item.id" class="duplicate-row">
                  ...
                  <div class="min-w-0 flex-1">
                    ...
                    <p class="mt-2 line-clamp-2 text-sm font-semibold text-slate-800 dark:text-slate-100">{{ item.questionText }}</p>
                  </div>
              ...
              <div v-if="duplicateGroup?.semanticCandidates?.length" class="mt-4 space-y-2">
                ...
                <article v-for="candidate in duplicateGroup.semanticCandidates" :key="candidate.question.id" class="duplicate-row semantic-candidate-row">
                  <div class="min-w-0 flex-1">
                    ...
                    <p class="mt-2 line-clamp-2 text-sm font-semibold text-slate-800 dark:text-slate-100">{{ candidate.question.questionText }}</p>
                  </div>
...
const setDuplicateCanonical = async (canonicalQuestionId: Question['id']) => {
  ...
  try {
    const res = await opsApi.setQuestionDuplicateCanonical(selectedQuestion.value.id, canonicalQuestionId)
    duplicateGroup.value = res.data || null
    duplicateSelection.value = []
...
const mergeDuplicateCandidate = async (candidateQuestionId: Question['id']) => {
  ...
  isDuplicateSaving.value = true
  try {
    const res = await opsApi.mergeQuestionDuplicateCandidate(selectedQuestion.value.id, candidateQuestionId)
    duplicateGroup.value = res.data || null
    duplicateSelection.value = []
