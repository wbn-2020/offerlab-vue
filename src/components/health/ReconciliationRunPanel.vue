...
<style scoped>
.reconcile-panel { display: grid; gap: 1rem; border: 1px solid var(--border-subtle); border-radius: .625rem; background: white; padding: 1rem; }
...
.panel-heading span:not(.meta-chip) { display: block; margin-top: .3rem; color: var(--text-muted); font-size: .73rem; line-height: 1.5; }
...
.meta-chip { background: var(--surface-soft); color: var(--text-primary); }
...
.state { display: flex; min-height: 6rem; flex-wrap: wrap; align-items: center; justify-content: center; gap: .5rem; border: 1px dashed var(--border-subtle); border-radius: .5rem; padding: 1rem; color: var(--text-muted); font-size: .75rem; text-align: center; }
...
.reconcile-form label > span { color: var(--text-primary); font-size: .68rem; font-weight: 800; }
.toggle-row { display: flex !important; min-height: 70px; grid-row: span 2; grid-template-columns: auto 1fr !important; align-items: center; gap: .55rem !important; border: 1px solid var(--border-subtle); border-radius: .5rem; padding: .65rem; }
.toggle-row input { height: 1rem; width: 1rem; }
.toggle-row span { display: grid; gap: .15rem; }
.toggle-row strong { color: var(--text-strong); font-size: .75rem; }
.toggle-row small { color: var(--text-muted); font-size: .65rem; font-weight: 500; line-height: 1.4; }
.reason-field { grid-column: 3; grid-row: span 2; }
.key-field { grid-column: 1 / span 2; }
.field-control { width: 100%; min-height: 36px; border: 1px solid var(--border-subtle); border-radius: .45rem; background: white; padding: .45rem .6rem; color: var(--text-strong); font-size: .72rem; resize: vertical; }
...
.primary-button { border: 1px solid rgb(26 127 90); background: rgb(26 127 90); color: white; }
.secondary-button { border: 1px solid var(--border-subtle); background: white; color: var(--text-primary); }
.result-panel { display: grid; gap: .8rem; border-top: 1px solid var(--border-subtle); padding-top: 1rem; }
...
.result-heading h3 { flex-basis: 100%; margin: .25rem 0 0; color: var(--text-strong); font-size: .85rem; font-weight: 900; }
.result-heading small { color: var(--text-muted); font-size: .66rem; }
.result-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: .5rem; }
.result-grid div { min-width: 0; border-radius: .5rem; background: var(--surface-soft); padding: .6rem; }
.result-grid dt, .result-grid dd { overflow-wrap: anywhere; }
.result-grid dt { color: var(--text-muted); font-size: .65rem; }
.result-grid dd { margin: .2rem 0 0; color: var(--text-strong); font-size: .78rem; font-weight: 900; }
.audit-copy { display: grid; grid-template-columns: 6rem minmax(0, 1fr); gap: .35rem .6rem; align-items: start; font-size: .67rem; }
.audit-copy span { color: var(--text-muted); font-weight: 800; }
.audit-copy code, .audit-copy p { margin: 0; color: var(--text-primary); overflow-wrap: anywhere; white-space: pre-wrap; }
...
.dark .reconcile-panel, .dark .field-control, .dark .toggle-row, .dark .secondary-button { border-color: var(--border-subtle); background: var(--surface-1); color: var(--text-muted); }
.dark .panel-heading h2, .dark .toggle-row strong, .dark .result-heading h3, .dark .result-grid dd { color: var(--text-strong); }
.dark .panel-heading span:not(.meta-chip), .dark .reconcile-form label > span, .dark .toggle-row small, .dark .state, .dark .result-heading small, .dark .result-grid dt, .dark .audit-copy span { color: var(--text-muted); }
.dark .meta-chip { background: var(--surface-1); color: var(--text-muted); }
.dark .result-panel { border-color: var(--border-subtle); }
.dark .result-grid div { background: color-mix(in srgb, var(--surface-1) 60%, transparent); }
.dark .audit-copy code, .dark .audit-copy p { color: var(--text-muted); }
</style>
