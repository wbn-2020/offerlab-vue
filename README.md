...
闻野（OfferLab）前端承载围绕真实经验、有用见闻、公共讨论和长期内容沉淀的综合内容社区。科技数码、学习成长、职场经验、生活方式和投资理财是并列频道；旧的题库、练习和学习空间能力仅作为历史兼容入口保留，不承担首页和一级导航心智。项目使用 Vue 3、Vite、TypeScript、Pinia、Vue Router、Axios、TailwindCSS、ECharts 和 Lucide 图标，当前主开发分支为 `dev-v2`。

## 项目能力

- 内容社区：首页、发现页、话题页、帖子详情、标签详情、Markdown 编辑器，以及跨频道的经验、问题、攻略、资源、观点、复盘和图文发布。
- 用户体系：登录、注册、当前用户、用户主页、我的主页、资料偏好和隐私设置。
- 互动体验：点赞、收藏、评论、评论点赞、通知中心和通知跳转。
- 搜索体验：关键词、频道、内容类型、高级兼容筛选、建议词、热词、URL 状态同步和搜索服务状态展示。
- 历史知识工具：知识卡、题目详情、主题学习包、个人学习空间和专项练习归档，不作为公共社区一级入口。
- 趋势看板：频道发布趋势、热门主题、高频标签、内容类型分布等真实后端统计数据展示。
- 公共共建：内容需求广场、协作合集、主题活动、频道策展和结构化讨论。
- 社区成长：区分不可消费的领域声望与逐步开放的野点，提供低风险虚拟权益、感谢票、平台配额悬赏和社区角色申请，不支持充值、提现或用户间转账。
...
| `/` | 首页 | 推荐内容与核心入口 |
| `/explore` | 发现 | 内容浏览 |
| `/collaboration` | 公共共建 | 内容需求、协作合集、主题活动和策展 |
| `/trend` | 趋势看板 | 真实统计数据 |
...
- `/api/v1/collaboration/**`：内容需求、协作合集、主题活动、策展和结构化讨论。
- `/api/v1/incentives/**`、`/api/v1/benefits/**`：声望、野点、虚拟权益、感谢票、平台悬赏和社区角色。
